/**
 * @file	fs/vdfs2/inode_squashfs.c
...
static int vdfs2_packtree_file_release(struct inode *inode, struct file *file)
{
	struct vdfs2_inode_info *inode_info = VDFS2_I(inode);
...
const struct file_operations vdfs2_packtree_fops = {
	...
	.open		= vdfs2_packtree_file_open,
	.release	= vdfs2_packtree_file_release,
};

/* todo */
void vdfs2_get_chunk_offset(struct installed_packtree_info *packtree_info,
		__u32 chunk_index, struct chunk_info *result, int is_frag)
{
	...
	VDFS2_BUG_ON((packtree_info->chunk_table[chunk_index] & 1) != is_frag);
}
	...
	char *obuff, int olen, int compr_type)
{
	...
	if ((rc != Z_OK) && (rc != Z_STREAM_END)) {
		...
		rc = (rc == Z_NEED_DICT) ? VDFS2_Z_NEED_DICT_ERR : rc;
		return rc;
	}
...
static int get_chunk_start_sector(struct installed_packtree_info *ptree_info,
	...
	struct vdfs2_inode_info *image_inode_info =
			VDFS2_I(ptree_info->tree->inode);
	struct chunk_info chunk;
	struct vdfs2_sb_info *sbi = VDFS2_SB(ptree_info->tree->inode->i_sb);
	__u32 length_in_blocks;
	sector_t iblock_requested;
	int i;

	vdfs2_get_chunk_offset(ptree_info, chunk_index, &chunk, is_fragment);
	...
	*clen = chunk.length;
	...
	for (i = 0; i < image_inode_info->fork.used_extents; i++) {
		struct vdfs2_extent_info *x = &image_inode_info->fork.extents[i];
		...
		int unzip_error)
{
	...
	printk(KERN_ERR "== VDFS2 Debugger - %15s ===== Core : %2d ====\n"
			, VDFS2_VERSION, current_thread_info()->cpu);
#endif
	printk(KERN_ERR "--------------------------------------------------\n");
	...
	struct inode *inode, __u32 chunk_index, __u32 index, int is_fragment)
{
...
#ifdef CONFIG_VDFS2_HW2_SUPPORT
	if ((VDFS2_SB(packed_inode->i_sb)->use_hw_decompressor) &&
	...
	ret = vdfs2_read_or_create_pages(VDFS2_SB(packed_inode->i_sb),
		packed_inode, packed_pages, packed_page_index,
		packed_page_count, 0);
	if (ret) {
		VDFS2_ERR("vdfs2_read_or_create_pages() returned error");
	...
	if (chunk.compressed) {
...
#ifdef CONFIG_VDFS2_SW_DECOMPRESS_RETRY
		unzip_retries = 3;
#else
		unzip_retries = 0;
#endif
		...
		if (ret)
			VDFS2_ERR("sw_decompression failed, see log above\n");
	} else
		...
		__u32 offset, __u32 length)
{
	struct installed_packtree_info *ptree_info =
			VDFS2_I(page->mapping->host)->ptree.tree_info;
...
static int vdfs2_read_inline_file_page(struct page *page)
{
	...
	if (!PageUptodate(page)) {
		...
		memcpy(page_data, VDFS2_I(inode)->ptree.tiny.data,
			inode->i_size);
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 5)
...
static int vdfs2_packtree_readpage(struct file *file, struct page *page)
{
	...
	if (VDFS2_I(inode)->record_type == VDFS2_CATALOG_PTREE_FILE_INLINE) {
		rc = vdfs2_read_inline_file_page(page);
		if (rc)
			goto out;
	...
	if (VDFS2_I(inode)->record_type == VDFS2_CATALOG_PTREE_FILE_FRAGMENT) {
		rc = get_packtree_data_page(page,
			VDFS2_I(inode)->ptree.frag.chunk_num,
			VDFS2_I(inode)->ptree.frag.unpacked_offset,
			i_size_read(inode));
		if (rc)
			goto out;
		...
		unlock_page(page);
	/*one big file*/
	} else if (VDFS2_I(inode)->record_type ==
			VDFS2_CATALOG_PTREE_FILE_CHUNKS) {
		int chunk_index = VDFS2_I(inode)->ptree.chunk.chunk_index +
				(page->index >> TOTAL_PAGES_SHIFT);
		/*if (chunk_info->data_start == 0xFFFFFFFF)  empty file
			goto pass;*/

		unlock_page(page);
		rc = read_and_unpack_chunk(VDFS2_I(inode)->ptree.tree_info,
			inode, chunk_index,
		...
		unsigned nr_pages)
{
	...
	__u16 sq_block_size_shift = VDFS2_I(inode)->ptree.tree_info->
				params.pmc.squash_bss;
	__u32 chunk_index = VDFS2_I(inode)->ptree.frag.chunk_num;
	struct installed_packtree_info *ptree_info =
			VDFS2_I(inode)->ptree.tree_info;
 ...
 * @return		Returns error codes
 */
static int vdfs2_packtree_readpages(struct file *filp,
...
again2:
	...
	if (VDFS2_I(inode)->record_type == VDFS2_CATALOG_PTREE_FILE_CHUNKS) {
		int chunk_index = VDFS2_I(inode)->ptree.chunk.chunk_index +
	...
	if (VDFS2_I(inode)->record_type == VDFS2_CATALOG_PTREE_FILE_INLINE) {
		ret = vdfs2_read_inline_file_page(pages_for_vmap[0]);
		kfree(pages_for_vmap);
		return ret;
	}
 ...
 */
static int vdfs2_read_pack_common(struct inode *inode,
		struct vdfs2_cattree_key *key)
{
	...
	memset(&VDFS2_I(inode)->ptree, 0, sizeof(VDFS2_I(inode)->ptree));

	VDFS2_I(inode)->record_type = key->record_type;

	VDFS2_I(inode)->flags = 0; /* todo */
	...
	i_uid_write(inode, le32_to_cpu(common->permissions.uid));
	i_gid_write(inode, le32_to_cpu(common->permissions.gid));
	...
	VDFS2_I(inode)->ptree.xattr.offset = le64_to_cpu(common->xattr_offset);
	VDFS2_I(inode)->ptree.xattr.size = le32_to_cpu(common->xattr_size);
	VDFS2_I(inode)->ptree.xattr.count = le32_to_cpu(common->xattr_count);
 ...
 * @return			Returns 0 on success, errno on failure
 */
static void vdfs2_read_pack_insert_point_value(struct inode *inode,
		struct vdfs2_cattree_key *key)
{
	struct vdfs2_inode_info *inode_info = VDFS2_I(inode);
 ...
 * @return			Returns 0 on success, errno on failure
 */
static int vdfs2_read_pack_file_tiny_value(struct inode *inode,
		struct vdfs2_cattree_key *key)
{
	struct vdfs2_pack_file_tiny_value *ftv =
		(struct vdfs2_pack_file_tiny_value *)get_value_pointer(key);
	if (inode->i_size > VDFS2_PACK_MAX_INLINE_FILE_SIZE)
		return -EFAULT;
	memcpy(VDFS2_I(inode)->ptree.tiny.data, ftv->data, inode->i_size);
	return 0;
}
 ...
 * @return			Returns 0 on success, errno on failure
 */
static int vdfs2_read_pack_symlink_value(struct inode *inode,
		struct vdfs2_cattree_key *key)
{
	struct vdfs2_pack_symlink_value *psv =
		(struct vdfs2_pack_symlink_value *) get_value_pointer(key);

	VDFS2_I(inode)->ptree.symlink.data = kzalloc(inode->i_size + 1,
		GFP_NOFS);
	if (!VDFS2_I(inode)->ptree.symlink.data)
		return -ENOMEM;
	memcpy(VDFS2_I(inode)->ptree.symlink.data, psv->data, inode->i_size);
	VDFS2_I(inode)->ptree.symlink.data[inode->i_size] = 0;
	return 0;
}
 ...
 * @return			Returns 0 on success, errno on failure
 */
static void vdfs2_read_pack_file_fragment_value(struct inode *inode,
		struct vdfs2_cattree_key *key)
{
	struct vdfs2_inode_info *inode_info = VDFS2_I(inode);
	struct vdfs2_pack_file_fragment_value *ffv =
		(struct vdfs2_pack_file_fragment_value *)get_value_pointer(key);
 ...
 * @return			Returns 0 on success, errno on failure
 */
static int vdfs2_read_pack_file_chunk_value(struct inode *inode,
		struct vdfs2_cattree_key *key)
{
	struct vdfs2_pack_file_chunk_value *fcv =
		(struct vdfs2_pack_file_chunk_value *)get_value_pointer(key);
	VDFS2_I(inode)->ptree.chunk.chunk_index =
			...
			struct vdfs2_cattree_key *key)
{
	...
	ret = vdfs2_read_pack_common(inode, key);
	if (ret)
		return ret;

	if (rec_type == VDFS2_CATALOG_PTREE_ROOT) {
		vdfs2_read_pack_insert_point_value(inode, key);
		inode->i_op = &vdfs2_packtree_root_dir_inode_operations;
		...
		ret = vdfs2_read_pack_symlink_value(inode, key);
		inode->i_op = &vdfs2_packtree_symlink_inode_operations;
		...
		if (rec_type == VDFS2_CATALOG_PTREE_FILE_INLINE)
			ret = vdfs2_read_pack_file_tiny_value(inode, key);
		else if (rec_type == VDFS2_CATALOG_PTREE_FILE_FRAGMENT)
			vdfs2_read_pack_file_fragment_value(inode, key);
		else if (rec_type == VDFS2_CATALOG_PTREE_FILE_CHUNKS)
			ret = vdfs2_read_pack_file_chunk_value(inode, key);
		else
			BUG();
	...
	void *buffer, size_t buf_size)
{
	...
	ret = vdfs2_read_or_create_pages(sbi, ptree_info->tree->inode,
...
/**
 * The vdfs2 packtree root directory inode operations.
 */
static const struct inode_operations vdfs2_packtree_root_dir_inode_operations = {
	.lookup		= vdfs2_lookup,
...
/**
 * The vdfs2 packtree directory inode operations.
 */
static const struct inode_operations vdfs2_packtree_dir_inode_operations = {
	.lookup		= vdfs2_lookup,
