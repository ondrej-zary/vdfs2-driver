...
#if defined(CONFIG_VDFS2_CRC_CHECK)
inline __le32 __vdfs2_get_checksum_offset(struct vdfs2_bnode *node)
{
	return node->host->node_size_bytes - VDFS2_BNODE_FIRST_OFFSET;
}

inline __le32 *__vdfs2_get_checksum_addr(struct vdfs2_bnode *node)
{
	return node->data + __vdfs2_get_checksum_offset(node);
}

inline void __vdfs2_update_checksum(struct vdfs2_bnode *node)
{
	*__vdfs2_get_checksum_addr(node) = crc32(0, node->data,
			__vdfs2_get_checksum_offset(node));
}

inline __le32 __vdfs2_get_checksum(struct vdfs2_bnode *node)
{
	return *__vdfs2_get_checksum_addr(node);
}

inline __le32 __vdfs2_get_checksum_offset_from_btree(struct vdfs2_btree
		*btree)
{
	return btree->node_size_bytes -	VDFS2_BNODE_FIRST_OFFSET;
}

inline __le32 *__vdfs2_get_checksum_addr_from_data(void *bnode_data,
		struct vdfs2_btree *btree)
{
	return bnode_data + __vdfs2_get_checksum_offset_from_btree(btree);
}

inline __le32 __vdfs2_calc_crc_for_bnode(struct vdfs2_bnode *bnode)
{
	return crc32(0, bnode->data, __vdfs2_get_checksum_offset(bnode));
}
inline void vdfs2_calc_crc_for_bnode(void *bnode_data,
		struct vdfs2_btree *btree)
{
	*__vdfs2_get_checksum_addr_from_data(bnode_data, btree) =
		crc32(0, bnode_data, __vdfs2_get_checksum_offset_from_btree(btree));
}
#endif
 ...
 */
static struct vdfs2_bnode *__get_bnode(struct vdfs2_btree *btree,
	__u32 node_id, int create, enum vdfs2_get_bnode_mode mode)
{
	struct inode *inode = btree->inode;
	struct vdfs2_bnode *node;
	...
	node = kzalloc(sizeof(struct vdfs2_bnode), GFP_KERNEL);
	if (!node)
		return ERR_PTR(-ENOMEM);

	page_index = node_id * btree->pages_per_node;
	if (btree->btree_type == VDFS2_BTREE_PACK)
		page_index += btree->tree_metadata_offset;

	pages = kzalloc(sizeof(*pages) * btree->pages_per_node, GFP_KERNEL);
	if (!pages) {
		struct vdfs2_bnode *ret = ERR_PTR(-ENOMEM);
		kfree(node);
		return ret;
	}

	ret = vdfs2_read_or_create_pages(btree->sbi, btree->inode, pages,
	...
	if (!node->data) {
		VDFS2_ERR("unable to vmap %d pages", btree->pages_per_node);
	...
	magic = node->data;
	if (!create && node_id != 0 && *magic != VDFS2_NODE_DESCR_MAGIC) {
...
#if defined(CONFIG_VDFS2_CRC_CHECK)
	if (create)
		__vdfs2_update_checksum(node);
	if ((!PageDirty(node->pages[0]) &&
			(btree->btree_type != VDFS2_BTREE_PACK))) {
		__le32 from_disk = __vdfs2_get_checksum(node);
		__le32 calculated = __vdfs2_calc_crc_for_bnode(node);
		if (from_disk != calculated) {
			VDFS2_ERR("Btree Inode id: %lu Bnode id: %u",
					btree->inode->i_ino, node_id);
			VDFS2_ERR("Expected %u, recieved: %u",
...
static int bnode_hash_fn(__u32 bnode_id)
{
	...
	return bnode_id & VDFS2_BNODE_HASH_MASK;
}

static void remove_from_lru(struct vdfs2_bnode *bnode)
{
...
static void add_to_passive(struct vdfs2_bnode *bnode)
{
#if VDFS2_PASSIVE_SIZE
	struct vdfs2_btree *btree = bnode->host;
	...
	btree->passive_use_count++;
	if (btree->passive_use_count > VDFS2_PASSIVE_SIZE) {
		/* Remove last element*/
		vdfs2_put_cache_bnode(list_entry(btree->passive_use.prev,
				struct vdfs2_bnode, lru_node));
		btree->passive_use_count--;
...
static void passive_to_active(struct vdfs2_bnode *bnode);

static void add_to_active(struct vdfs2_bnode *bnode)
{
	struct vdfs2_btree *btree = bnode->host;
#if VDFS2_ACTIVE_SIZE

	list_add(&bnode->lru_node, &btree->active_use);
	bnode->state = 1;

	btree->active_use_count++;
	if (btree->active_use_count > VDFS2_ACTIVE_SIZE) {
		struct vdfs2_bnode *deleting =
...
static void hash_bnode(struct vdfs2_bnode *bnode)
{
	struct vdfs2_btree *btree = bnode->host;
...
static void unhash_bnode(struct vdfs2_bnode *bnode)
{
...
static struct vdfs2_bnode *get_bnode_from_hash(struct vdfs2_btree *btree,
		__u32 bnode_id, enum vdfs2_get_bnode_mode mode)
{
	int hash = bnode_hash_fn(bnode_id);
	struct hlist_node *iterator;

	hlist_for_each(iterator, &btree->hash_table[hash]) {
		struct vdfs2_bnode *node = hlist_entry(iterator,
				struct vdfs2_bnode, hash_node);
...
#ifdef CONFIG_VDFS2_DEBUG_GET_BNODE
static int save_get_bnode_stack(struct vdfs2_bnode *bnode)
{
	struct vdfs2_get_bnode_trace *bnode_trace =
	...
	bnode_trace->trace.max_entries = VDFS2_GET_BNODE_STACK_ITEMS;
...
static void print_get_bnode_stack(struct vdfs2_bnode *bnode)
{
	struct vdfs2_get_bnode_trace *trace;
	int i = 0;
...
static void free_get_bnode_stack(struct vdfs2_bnode *bnode)
{
	struct vdfs2_get_bnode_trace *trace, *tmp;
...
#ifdef CONFIG_VDFS2_DEBUG_GET_BNODE
		if (val > 2) {
			VDFS2_DEBUG_TMP("val is %d", val);
			print_get_bnode_stack(bnode);
			VDFS2_BUG();
		}
#else
		VDFS2_BUG_ON(mode == VDFS2_BNODE_MODE_RW && val > 2);
#endif
	}
...
{
	struct vdfs2_bnode_bitmap *bitmap = btree->bitmap;
	struct vdfs2_bnode *bnode;
	...
	VDFS2_BUG_ON(test_and_set_bit(bitmap->first_free_id, bitmap->data));
...
static void free_bnode(struct vdfs2_bnode *bnode)
{
	unsigned int i;
 ...
 * @return	void
 */
static void __put_bnode(struct vdfs2_bnode *bnode, int keep_in_cache)
{
	VDFS2_BUG_ON(!bnode->data || !bnode->pages);
 ...
 * */
static int vdfs2_prealloc_one_bnode_reserve(struct vdfs2_btree *btree,
		__u32 reserving_bnode_id)
{
	struct vdfs2_bnode *bnode;

	bnode = __get_bnode(btree, reserving_bnode_id, 1, VDFS2_BNODE_MODE_RW);
	if (IS_ERR(bnode))
		return PTR_ERR(bnode);

	VDFS2_BUG_ON(atomic_dec_return(&bnode->ref_count));
	vdfs2_temp_stub_init_new_node_descr(bnode, 0);
...
int vdfs2_check_bnode_reserve(struct vdfs2_btree *btree)
{
	...
	available_bnodes = VDFS2_LAST_TABLE_INDEX(btree->sbi,
			btree->inode->i_ino) + 1;
	...
	bnodes_deficit = (long int) VDFS2_BNODE_RESERVE(btree) - free_bnodes;
	...
	for (i = available_bnodes; i < new_bnodes_num; i++) {
		ret = vdfs2_prealloc_one_bnode_reserve(btree, i);
		if (ret)
			break;
	}
...
int vdfs2_destroy_bnode(struct vdfs2_bnode *bnode)
{
	...
	VDFS2_BUG_ON(bnode_id == VDFS2_INVALID_NODE_ID);
	...
	/* TODO - the comment same as at vdfs2_alloc_new_bnode */
	vdfs2_mark_bnode_dirty(btree->head_bnode);
...
{
	VDFS2_BUG_ON(!node);
	VDFS2_BUG_ON(node->mode != VDFS2_BNODE_MODE_RW);
	vdfs2_add_chunk_bnode(node->host->sbi, node->pages);
}
...
#if defined(CONFIG_VDFS2_META_SANITY_CHECK)
	vdfs2_meta_sanity_check_bnode(bnode_data, btree, page[0]->index);
#endif
	memcpy(bnode_data + 4, &version, VERSION_SIZE);
#if defined(CONFIG_VDFS2_CRC_CHECK)
	vdfs2_calc_crc_for_bnode(bnode_data, btree);
#endif
	vunmap(bnode_data);
	return 0;
}
