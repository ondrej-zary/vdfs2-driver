 ...
 */
static struct kmem_cache *vdfs2_fsm_node_cachep;
 ...
 */
struct vdfs2_fsm_btree_node {
	/** Pointer used for linked list */
	struct vdfs2_fsm_btree_node *next;
	...
	struct fsm_btree_key key_vals[2 * VDFS2_FSM_BTREE_ORDER - 1];

	/** Array of pointers to child nodes */
	struct vdfs2_fsm_btree_node *children[2 * VDFS2_FSM_BTREE_ORDER];
};
...
{
	memset(node, 0, sizeof(struct vdfs2_fsm_btree_node));
}
...
{
	vdfs2_fsm_node_cachep = kmem_cache_create("vdfs2_fsm_btree",
		sizeof(struct vdfs2_fsm_btree_node), 0,
		SLAB_HWCACHE_ALIGN, btree_node_ctor);

	if (!vdfs2_fsm_node_cachep) {
		VDFS2_ERR("failed to initialize inode cache\n");
		return -ENOMEM;
	}
	return 0;
}
...
{
	kmem_cache_destroy(vdfs2_fsm_node_cachep);
}
...
{
	VDFS2_DEBUG_FSM("PRINT off %llu\tlen %llu ",
...
{
	btree->order = VDFS2_FSM_BTREE_ORDER;
	btree->root = NULL;
 ...
 */
static struct vdfs2_fsm_btree_node *allocate_btree_node(
 ...
 * @return	void
 */
static void free_btree_node(struct vdfs2_fsm_btree *tree,
	struct vdfs2_fsm_btree_node *node)
{
	kmem_cache_free(vdfs2_fsm_node_cachep, node);
	node = NULL;
}
 ...
 */
static int btree_split_child(struct vdfs2_fsm_btree *btree,
			struct vdfs2_fsm_btree_node *parent,
			unsigned int index,
			struct vdfs2_fsm_btree_node *child)
{
	unsigned int count = 0;
	unsigned int order = btree->order;

	struct vdfs2_fsm_btree_node *new_child = allocate_btree_node(btree);
 ...
 * @return			Returns 0 on success, ERRNO on fail.
 */
static int btree_insert_nonfull(struct vdfs2_fsm_btree *btree,
		struct vdfs2_fsm_btree_node *parent_node, void *key_val)
{
	...
	int err = 0;
	struct vdfs2_fsm_btree_node *child;
	struct vdfs2_fsm_btree_node *node = parent_node;
 ...
 */
int fsm_btree_insert(struct vdfs2_fsm_btree *btree, void *key_val)
{
	struct vdfs2_fsm_btree_node *rnode = NULL;
	int ret = -1;
	...
	if (rnode->nr_active == (2*btree->order - 1)) {
		struct vdfs2_fsm_btree_node *new_root;
		new_root = allocate_btree_node(btree);
		if (unlikely(IS_ERR(new_root)))
			return -ENOMEM;
 ...
 */
static struct vdfs2_fsm_btree_node *get_max_key_pos(
	struct vdfs2_fsm_btree *btree,
	struct vdfs2_fsm_btree_node *nd)
{

	struct vdfs2_fsm_btree_node *node = nd, *ret_node = NULL;
 ...
 * @return		Returns node containing the key and position of key.
 */
static struct vdfs2_fsm_btree_node *get_min_key_pos(
	struct vdfs2_fsm_btree *btree,
	struct vdfs2_fsm_btree_node *nd)
{
	struct vdfs2_fsm_btree_node *node = nd, *ret_node = NULL;
 ...
 */
static struct vdfs2_fsm_btree_node *merge_siblings(
		struct vdfs2_fsm_btree *btree,
		struct vdfs2_fsm_btree_node *parent,
		unsigned int index , enum position_t pos)
{
	unsigned int j;
	struct vdfs2_fsm_btree_node *new_node;
	struct vdfs2_fsm_btree_node *n1, *n2;
 ...
 * @return	void
 */
static void move_key(struct vdfs2_fsm_btree *btree,
		struct vdfs2_fsm_btree_node *node,
		unsigned int index, enum position_t pos)
{
	struct vdfs2_fsm_btree_node *lchild;
	struct vdfs2_fsm_btree_node *rchild;
	unsigned int i;
 ...
 */
static struct vdfs2_fsm_btree_node *merge_nodes(
		struct vdfs2_fsm_btree *btree,
		struct vdfs2_fsm_btree_node *n1,
		struct fsm_btree_key kv,
		struct vdfs2_fsm_btree_node *n2)
{
	struct vdfs2_fsm_btree_node *new_node;
	unsigned int i;
 ...
 */
static int remove_key_from_leaf(struct vdfs2_fsm_btree *btree,
			 struct vdfs2_fsm_btree_node *node, unsigned int pos)
{
 ...
 * @return		Returns 0 on success, -1 on error.
 */
int fsm_btree_delete(struct vdfs2_fsm_btree *btree, void *key)
{
	unsigned int i, index;
	struct vdfs2_fsm_btree_node *node = NULL, *rsibling, *lsibling;
	struct vdfs2_fsm_btree_node *comb_node, *parent;

	struct vdfs2_fsm_btree_node *sub_np_node;
	int sub_np_pos;
 ...
 * @return	void
 */
void fsm_btree_destroy(struct vdfs2_fsm_btree *btree)
{
	unsigned i = 0;
	unsigned int current_level;

	struct vdfs2_fsm_btree_node *head, *tail, *node;
	struct vdfs2_fsm_btree_node *child, *del_node;
 ...
 */
int fsm_btree_free(struct vdfs2_fsm_btree *btree, __u64 block_offset,
	__u64 length)
{
	unsigned i;
	int ret = 0;
	struct vdfs2_fsm_btree_node *node = btree->root;
 ...
 * @param [in,out]	length	Length key value (size of continuous
 *				free block sequence to find).
 * @param [in]	can_return_less	If not 0, function can return less than length
 *				free block space.
 * @return			Returns physical block number of the required
 *				continuous sequence of free blocks.
 */
__u64 fsm_btree_lookup(struct vdfs2_fsm_btree *btree, __u64 block_offset,
	__u32 *length, int can_return_less)
{
	unsigned int i;
	struct vdfs2_fsm_btree_node *node = btree->root;
	struct vdfs2_fsm_btree_node *tail;
	...
	while (true) {
		...
		for (i = 0; i < node->nr_active; i++) {
			if (*length < node->key_vals[i].length) {
				...
				key.block_offset = block_offset =
					node->key_vals[i].block_offset;
				key.length = *length = node->key_vals[i].length;
				ret = fsm_btree_delete(btree, &key);
				return ret ? 0L : block_offset;
			}
		}
 ...
 * @return	void
 */
static void print_single_node(struct vdfs2_fsm_btree *btree,
	struct vdfs2_fsm_btree_node *node)
{
	unsigned int i = 0;
	...
	VDFS2_DEBUG_FSM("} (0x%p,cnt %d, level %d type %s) ",
 ...
 * @return	void
 */
void fsm_print_tree(struct vdfs2_fsm_btree *btree)
{
	unsigned int i = 0;
	unsigned int current_level;

	struct vdfs2_fsm_btree_node *head, *tail, *node = btree->root;

	if (!node) {
		VDFS2_DEBUG_FSM("DEBUG: Subtree is empty\n");
		return;
	}
	current_level = node->level;
	head = node;
	...
	VDFS2_DEBUG_FSM("DEBUG: Printing subtree\n");
	while (true) {
		if (head == NULL)
			break;
		if (head->level < current_level) {
