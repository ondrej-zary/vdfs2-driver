...
#define VDFS2_MAX_FILE_PATH_LEN 1024
struct proc_dir_entry *vdfs2_procdir_entry;

struct vdfs2_proc_dir_info {
...
struct vdfs2_proc_file_struct {
	char path[VDFS2_MAX_FILE_PATH_LEN];
	char *printing_path;
	struct list_head list_entry;
};

void print_path(struct vdfs2_proc_file_struct *vdfs2_file, struct seq_file *m)
{
	if (vdfs2_file->printing_path)
		seq_printf(m, "%s\n", vdfs2_file->printing_path);
}

static void *vdfs2_file_paths_start(struct seq_file *m, loff_t *pos)
{
	struct vdfs2_sb_info *sbi;
	struct file *f;
	struct vdfs2_proc_file_struct *new_entry;
	...
	do_file_list_for_each_entry(sbi->sb, f) {
		...
		if (dirty_pages_count) {
			new_entry = kzalloc(sizeof(struct vdfs2_proc_file_struct),
							GFP_KERNEL);
			if (!new_entry)
				return NULL;
	...
	return list_entry(sbi->proc_info->files_list.next,
			struct vdfs2_proc_file_struct, list_entry);
}

static void *vdfs2_file_paths_next(struct seq_file *m, void *v, loff_t *pos)
{
	struct vdfs2_proc_file_struct *file = v;
	struct vdfs2_sb_info *sbi = m->private;
...
static void vdfs2_file_paths_stop(struct seq_file *m, void *v)
{
	struct vdfs2_sb_info *sbi;
	struct vdfs2_proc_file_struct *file;
	struct vdfs2_proc_file_struct *file_next;
...
static int vdfs2_file_paths_show(struct seq_file *m, void *v)
{
	print_path(v, m);
	return 0;
}
...
static const struct seq_operations seq_ops = {
	.start = vdfs2_file_paths_start,
	.next = vdfs2_file_paths_next,
	.stop = vdfs2_file_paths_stop,
	.show = vdfs2_file_paths_show
};

static int vdfs2_file_paths_open(struct inode *inode, struct file *file)
{
	struct seq_file *s;
	int result = seq_open(file, &seq_ops);
	struct proc_dir_entry *pde = PDE(inode);
	struct vdfs2_sb_info *sbi = pde->data;
...
int vdfs2_build_proc_entry(struct vdfs2_sb_info *sbi)
{
	...
	sbi->proc_info->device_directory_entry =
		proc_mkdir(sbi->sb->s_id, vdfs2_procdir_entry);
...
cannot_create_proc_entry:
	remove_proc_entry(sbi->sb->s_id, vdfs2_procdir_entry);
...
{
	vdfs2_procdir_entry = proc_mkdir(VDFS2_PROC_DIR_NAME, NULL);
	if (!vdfs2_procdir_entry)
		return -EINVAL;
...
{
	remove_proc_entry(VDFS2_PROC_DIR_NAME, NULL);
}
