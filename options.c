...
/**
 * The VDFS2 mount options match tokens.
 */
...
int vdfs2_parse_options(struct super_block *sb, char *input)
{
	...
	set_option(VDFS2_SB(sb), VERCHECK);
	...
	while ((p = strsep(&input, ",")) != NULL) {
		...
		case option_novercheck:
			clear_option(VDFS2_SB(sb), VERCHECK);
			printk(KERN_WARNING "[VDFS2-warning] Checking versions"
					" of driver and mkfs is disabled\n");

			break;

		case option_btreecheck:
			set_option(VDFS2_SB(sb), BTREE_CHECK);
			break;

		case option_debugcheck:
			set_option(VDFS2_SB(sb), DEBUG_AREA_CHECK);
			break;
		case option_tiny:
			set_option(VDFS2_SB(sb), TINY);
			break;
		case option_tinysmall:
			set_option(VDFS2_SB(sb), TINYSMALL);
			break;
		case option_count:
			if (match_int(&args[0], &option))
				BUG();

			VDFS2_DEBUG_TMP("counter %d", option);
			VDFS2_SB(sb)->bugon_count = option;
			break;
		...
		case option_fmask:
			...
			if (ret) {
				printk(KERN_ERR "[VDFS2-ERROR] fmask must be "
						"octal-base value");
				return ret;
			}
			if (option & ~((unsigned int) S_IRWXUGO)) {
				printk(KERN_ERR "[VDFS2-ERROR] fmask  "
					" is wrong");
				return -EINVAL;
			}
			set_option(VDFS2_SB(sb), FMASK);
			VDFS2_SB(sb)->fmask = option;
			break;
		case option_dmask:
			ret = match_octal(&args[0], &option);
			if (ret) {
				printk(KERN_ERR "[VDFS2-ERROR] dmask must be "
						"octal-base value");
				return ret;
			}
			if (option & ~((unsigned int) S_IRWXUGO)) {
				printk(KERN_ERR "[VDFS2-ERROR] dmask  "
					" is wrong");
				return -EINVAL;
			}

			set_option(VDFS2_SB(sb), DMASK);
			VDFS2_SB(sb)->dmask = option;
			break;
		default:
			return -EINVAL;
		}
