 ...
 */
static inline int rb_compare_keys(struct vdfs2_indirect_key *l_key,
		struct vdfs2_indirect_key *r_key)
{
 ...
 */
static inline struct vdfs2_nfs_file *rb_search_for_file(struct rb_root *root,
		struct vdfs2_indirect_key *indirect_key)
{
	struct rb_node **new = &(root->rb_node);
	struct vdfs2_nfs_file *this = NULL;
	int cmp_res = 0;

	/* go through all item */
	while (*new) {
		this =  rb_entry(*new, struct vdfs2_nfs_file, node);
		...
		struct vdfs2_nfs_file *file)
{
	...
	while (*new) {
		struct vdfs2_nfs_file *this =  rb_entry(*new,
				struct vdfs2_nfs_file, node);
		parent = *new;
...
int vdfs2_get_record_type_on_mode(struct inode *inode, u8 *record_type)
{
	...
	if (is_vdfs2_inode_flag_set(inode, HARD_LINK))
		*record_type = VDFS2_CATALOG_HLINK_RECORD;
...
{
	struct vdfs2_catalog_folder_record *comm_rec = value_area;

	vdfs2_get_vfs_inode_flags(inode);
	comm_rec->flags = cpu_to_le32(VDFS2_I(inode)->flags);

	/* TODO - set magic */
/*	memcpy(comm_rec->magic, get_magic(le16_to_cpu(comm_rec->record_type)),
			sizeof(VDFS2_CAT_FOLDER_MAGIC) - 1);*/
	comm_rec->permissions.file_mode = cpu_to_le16(inode->i_mode);
	comm_rec->permissions.uid = cpu_to_le32(i_uid_read(inode));
	comm_rec->permissions.gid = cpu_to_le32(i_gid_read(inode));
	...
	if (S_ISLNK(inode->i_mode) || S_ISREG(inode->i_mode)) {
		struct vdfs2_catalog_file_record *file_rec = value_area;
		if (is_vdfs2_inode_flag_set(inode, TINY_FILE)) {
			file_rec->tiny.len = VDFS2_I(inode)->tiny.len;
			file_rec->tiny.i_size =
				cpu_to_le64(VDFS2_I(inode)->tiny.i_size);
			memcpy(file_rec->tiny.data, VDFS2_I(inode)->tiny.data,
						TINY_DATA_SIZE);
		} else if (is_vdfs2_inode_flag_set(inode, SMALL_FILE)) {
			file_rec->small.cell =
				cpu_to_le64(VDFS2_I(inode)->small.cell);
			file_rec->small.len =
				cpu_to_le16(VDFS2_I(inode)->small.len);
			file_rec->small.i_size =
				cpu_to_le64(VDFS2_I(inode)->small.i_size);
		} else
			vdfs2_form_fork(&file_rec->data_fork, inode);
	}
	return 0;
}
		...
		struct qstr *name)
{
	...
	VDFS2_BUG_ON(inode->i_ino < VDFS2_1ST_FILE_INO);
	...
	record = vdfs2_alloc_cattree_key(name->len, record_type);

	if (IS_ERR(record))
		return PTR_ERR(record);

	vdfs2_fill_cattree_key(record, parent_id, name->name, name->len);
	...
	if (vdfs2_remove_indirect_index(sbi->catalog_tree, inode) == 0) {
		/* add new */
		ret = vdfs2_add_indirect_index(sbi->catalog_tree,
	...
	if (!(is_vdfs2_inode_flag_set(inode, HARD_LINK)))
		...
		vdfs2_fill_hlink_value(inode, get_value_pointer(record));

	if (ret)
		goto exit;

	ret = vdfs2_btree_insert(sbi->catalog_tree, record);
	if (ret)
		goto exit;
exit:
	kfree(record);
...
{
	struct vdfs2_sb_info *sbi = VDFS2_SB(inode_i->vfs_inode.i_sb);
	struct vdfs2_cattree_key *key, *found_key;
	int ret = 0;


	VDFS2_BUG_ON(fd->bnode);
	...
	/*key->name.length = vdfs2_unicode_strlen(inode_i->name);*/
	...
	if (!sbi->catalog_tree->comp_fn(&key->gen_key, &found_key->gen_key)) {
		...
		VDFS2_DEBUG_INO("ino#%lu hint match bnode_id=%u pos=%d",
	...
	vdfs2_put_bnode(fd->bnode);
find:
	VDFS2_DEBUG_INO("ino#%lu moved from bnode_id=%lu pos=%d",
			inode_i->vfs_inode.i_ino,
			(long unsigned int) inode_i->bnode_hint.bnode_id,
			inode_i->bnode_hint.pos);

	fd->bnode = vdfs2_btree_find(sbi->catalog_tree, &key->gen_key,
			&fd->pos, mode);
		...
		__u64 ino, struct vdfs2_cattree_key *key)
{
	int ret = 0;
#ifdef CONFIG_VDFS2_NFS_SUPPORT
	struct vdfs2_nfs_file *nfs_file  = kzalloc(sizeof(struct vdfs2_nfs_file),
			GFP_KERNEL);
	if (nfs_file == NULL)
		return -ENOMEM;
...
{
#ifdef CONFIG_VDFS2_NFS_SUPPORT
	struct vdfs2_nfs_file *nfs_file =  NULL;
	struct vdfs2_cattree_record *file_record = NULL;
	struct inode *inode = NULL;
	int ret = 0;
	struct vdfs2_cattree_key *rec_key;
	...
	file_record = vdfs2_cattree_find(tree, rec_key->parent_id,
