...
int build_quota_manager(struct vdfs2_sb_info *sbi)
{
	sbi->quotas = kzalloc(INITIAL_QUOTA_ARRAY_SIZE *
			sizeof(struct vdfs2_quota), GFP_KERNEL);
	if (!sbi->quotas)
		return -ENOMEM;
...
{
	struct vdfs2_sb_info *sbi = VDFS2_SB(dentry->d_inode->i_sb);
	int index;
	char temp[UUL_MAX_LEN + 1];
	ssize_t rc;

	memset(temp, 0, UUL_MAX_LEN + 1);
	rc = vdfs2_getxattr(dentry, QUOTA_HAS_XATTR, temp, UUL_MAX_LEN);
	if (IS_ERR_VALUE(rc))
		return rc;
	...
	memset(temp, 0, UUL_MAX_LEN + 1);
	rc = vdfs2_getxattr(dentry, QUOTA_XATTR, temp, UUL_MAX_LEN);
	if (IS_ERR_VALUE(rc))
		return rc;
