...
{
	struct vdfs2_cattree_key *key1, *key2;
	__u8 *ch1, *ch2;
	int diff;
	int i;


	key1 = container_of(__key1, struct vdfs2_cattree_key, gen_key);
	key2 = container_of(__key2, struct vdfs2_cattree_key, gen_key);
 ...
 *				(used later in vdfs2_btree_find)
...
{
	struct vdfs2_cattree_key *key, *found_key;
	int ret = 0;

	if (fd->bnode) {
		VDFS2_ERR("bnode should be empty");
	...
	/*key->name.length = vdfs2_unicode_strlen(name);*/
	/*key2 = kzalloc(sizeof(*key2), GFP_KERNEL);*/
	key->name.length = len;
	memcpy(key->name.unicode_str, name, len);


	fd->bnode = __vdfs2_btree_find(tree, &key->gen_key,
			&fd->pos, mode);
	...
	if (fd->pos == -1) {
		VDFS2_ERR("parent_id=%llu name=%s", parent_id, name);
		if (!is_sbi_flag_set(tree->sbi, IS_MOUNT_FINISHED))
			ret = -EFAULT;
		else
			VDFS2_BUG();
	...
	found_key = vdfs2_get_btree_record(fd->bnode, fd->pos);
	if (IS_ERR(found_key)) {
	...
	if (tree->comp_fn(&key->gen_key, &found_key->gen_key)) {
		...
		/* TODO - ENOENT error is returned from vdfs2_get_bnode. Let's
 ...
 * old */
static void temp_fill_record(struct vdfs2_bnode *bnode, int pos,
		struct vdfs2_btree_record_info *rec_info)
{
	struct vdfs2_cattree_record *record = (void *) &rec_info->gen_record;

	rec_info->rec_pos.bnode = bnode;
	rec_info->rec_pos.pos = pos;
	record->key = vdfs2_get_btree_record(bnode, pos);

	BUG_ON(!record->key);
	if (IS_ERR(record->key)) {
		kfree(record);
		VDFS2_ERR("unable to get record");
		VDFS2_BUG();
	}
...
struct vdfs2_cattree_record *vdfs2_cattree_find(struct vdfs2_btree *btree,
		__u64 parent_id, const char *name, int len,
		enum vdfs2_get_bnode_mode mode)
{
	...
	record = (struct vdfs2_cattree_record *) vdfs2_btree_find(btree,
			&key->gen_key, mode);
	if (IS_ERR(record))
		goto exit;
...
{
	u32 record_len = 0;
	struct vdfs2_cattree_key *form_key;
	u32 key_len = sizeof(struct vdfs2_cattree_key) -
			sizeof(struct vdfs2_unicode_string) +
			sizeof(__le32) +
	...
	switch (record_type) {
	case VDFS2_CATALOG_FILE_RECORD:
		record_len = sizeof(struct vdfs2_catalog_file_record);
	break;
	case VDFS2_CATALOG_FOLDER_RECORD:
	case VDFS2_CATALOG_RECORD_DUMMY:
		record_len = sizeof(struct vdfs2_catalog_folder_record);
	break;
	case VDFS2_CATALOG_HLINK_RECORD:
		record_len = sizeof(struct vdfs2_catalog_hlink_record);
	break;
	case VDFS2_CATALOG_PTREE_ROOT:
		record_len = sizeof(struct vdfs2_pack_insert_point_value);
	break;
	default:
		VDFS2_BUG();
	}
 ...
 * @return	void
 */
void vdfs2_fill_cattree_key(struct vdfs2_cattree_key *fill_key,
		...
		const char *name, int len, u8 record_type)
{
	...
	insert_data = vdfs2_alloc_cattree_key(len, record_type);

	if (IS_ERR(insert_data))
		return insert_data;

	vdfs2_fill_cattree_key(insert_data, parent_id, name, len);

	err = vdfs2_btree_insert(tree, insert_data);
	kfree(insert_data);

	if (err)
		return ERR_PTR(err);

	return vdfs2_cattree_find(tree, parent_id, name, len,
			VDFS2_BNODE_MODE_RW);
}
...
struct vdfs2_cattree_record *vdfs2_cattree_get_first_child(
	struct vdfs2_btree *btree, __u64 catalog_id)
{
	struct vdfs2_cattree_key *search_key;
	struct vdfs2_cattree_record *record;
	int ret;
	...
	record = (struct vdfs2_cattree_record *) vdfs2_btree_find(btree,
			&search_key->gen_key, VDFS2_BNODE_MODE_RO);
	if (IS_ERR(record))
		goto exit;
	...
	ret = vdfs2_get_next_btree_record((struct vdfs2_btree_gen_record *)
			record);
	if (ret) {
		vdfs2_release_record((struct vdfs2_btree_gen_record *) record);
		record = ERR_PTR(ret);
...
{
	if (key_type == VDFS2_CATALOG_FOLDER_RECORD)
		return sizeof(struct vdfs2_catalog_folder_record) +
			sizeof(struct vdfs2_cattree_key);

	if (key_type == VDFS2_CATALOG_FILE_RECORD)
		return sizeof(struct vdfs2_catalog_file_record) +
			sizeof(struct vdfs2_cattree_key);

	if (key_type == VDFS2_CATALOG_HLINK_RECORD)
		return sizeof(struct vdfs2_catalog_hlink_record) +
			sizeof(struct vdfs2_cattree_key);
...
int vdfs2_strcpy(struct  vdfs2_unicode_string *string, const char *cstring)
{
	u_int32_t str_len;

	str_len = strlen(cstring);
	if (str_len > VDFS2_FILE_NAME_LEN)
		return -ENAMETOOLONG;
 ...
 *				or error code otherwise
 */
struct vdfs2_cattree_key *vdfs2_alloc_cattree_record(int name_len,
		int record_type)
{
	u32 record_len = 0;
	struct vdfs2_cattree_key *form_key;
	...
	if (record_type == VDFS2_CATALOG_FILE_RECORD)
		record_len = sizeof(struct vdfs2_catalog_file_record);
	else if (record_type == VDFS2_CATALOG_FOLDER_RECORD)
		record_len = sizeof(struct vdfs2_catalog_folder_record);
...
int init_cattree_folder_record_value(
	struct vdfs2_catalog_folder_record *folder,
	u_int64_t total_items_count, u_int64_t links_count,
	u_int64_t uuid, struct vdfs2_posix_permissions	*permissions,
	const struct vdfs2_date *time_creation,
	const struct vdfs2_date *time_modification,
	const struct vdfs2_date *time_access)
{
	folder->flags = 0;
...
static int cattree_init_root_bnode(struct vdfs2_bnode *root_bnode)
{
	int ret = 0;
	const char *name = VDFS2_ROOTDIR_NAME;
	struct vdfs2_sb_info *sbi = root_bnode->host->sbi;
	struct vdfs2_catalog_folder_record *root_folder_value;
	struct vdfs2_posix_permissions perm_root;
	void *root_record;

	memset(root_bnode->data, 0x0, get_bnode_size(sbi));
	vdfs2_temp_stub_init_new_node_descr(root_bnode, VDFS2_NODE_LEAF);

	root_record = vdfs2_alloc_cattree_record(strlen(name),
			VDFS2_CATALOG_FOLDER_RECORD);
	if (IS_ERR(root_record))
		return PTR_ERR(root_record);

	vdfs2_fill_cattree_key(root_record, 0, name, strlen(name));
	...
	root_folder_value->object_id = VDFS2_ROOT_INO;
	...
	vdfs2_temp_stub_insert_into_node(root_bnode, root_record, 0);
exit:
	free(root_record);
	return ret;
}

int init_cattree(struct vdfs2_sb_info *sbi)
{
	int ret = 0;
	struct vdfs2_tools_btree_info *cattree_btree = &sbi->cattree;
	struct vdfs2_bnode *root_bnode = 0;

	log_activity("Create catalog tree");


	ret = btree_init(sbi, cattree_btree, VDFS2_BTREE_CATALOG,
			sizeof(struct vdfs2_cattree_key) +
			sizeof(struct vdfs2_catalog_file_record));
	if (ret)
		goto error_exit;


	/* Init root bnode */
	root_bnode = vdfs2_alloc_new_bnode(&cattree_btree->vdfs2_btree);
	...
	cattree_btree->tree.sub_system_id = VDFS2_CAT_TREE_INO;
	cattree_btree->vdfs2_btree.comp_fn = vdfs2_cattree_cmpfn;
...
u_int32_t catalog_tree_get_height(struct vdfs2_sb_info *sbi)
{
 ...
 * @param [in] block_size
  */
void fork_init(struct vdfs2_fork *_fork, u_int64_t begin, u_int64_t length,
		unsigned int block_size)
{
	int i ;
	memset(_fork, 0, sizeof(struct vdfs2_fork));
	_fork->magic = VDFS2_FORK_MAGIC;
 ...
 */
void vdfs2_fill_cattree_record_value(struct vdfs2_cattree_record *record,
		...
		unsigned int block_size)
{
	struct vdfs2_catalog_folder_record *val = VDFS2_CATTREE_FOLDVAL(record);
	struct vdfs2_catalog_hlink_record *hl =
	...
	if (record->key->record_type == VDFS2_CATALOG_FILE_RECORD) {
		struct vdfs2_catalog_file_record *file_rec = (
				struct vdfs2_catalog_file_record *) val;
...
struct vdfs2_bnode *get_host_bnode(struct vdfs2_cattree_record *record)
{
	struct vdfs2_btree_record_info *rec_info =
		VDFS2_BTREE_REC_I((void *) record);
