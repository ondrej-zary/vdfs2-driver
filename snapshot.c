...
static int expand_special_file(struct vdfs2_sb_info *sbi, ino_t ino_n,
		__u64 new_last_iblock);
static void insert_new_iblocks(struct vdfs2_sb_info *sbi, ino_t ino_n,
 ...
 */
static inline struct vdfs2_task_log *search_for_task_log(struct rb_root *root)
{
	struct task_struct *current_task = current;
	struct rb_node **new = &(root->rb_node);
	struct vdfs2_task_log *this = NULL;


	/* Figure out where to put new node */
	while (*new) {
		this = container_of(*new, struct vdfs2_task_log, node);
		...
		struct vdfs2_task_log *data)
{
	...
	/* Figure out where to put new node */
	while (*new) {
		struct vdfs2_task_log *this = container_of(*new,
				struct vdfs2_task_log, node);
 ...
 * @return		TODO Returns 0 on success, errno on failure.
 */
static struct vdfs2_task_log *get_task_log_start(struct vdfs2_sb_info *sbi)
{
	struct vdfs2_task_log *current_task_log;
	struct vdfs2_task_log *new_task_log;
	int ret;
	...
	if (!current_task_log) {
again:
		new_task_log = kzalloc(sizeof(struct vdfs2_task_log)
				, GFP_KERNEL);
		if (!new_task_log)
			goto again;
...
static inline void destroy_task_log(struct rb_root *root)
{
	...
	while (current_node) {
		...
		struct vdfs2_task_log *task_log;
 ...
 * @return		TODO Returns 0 on success, errno on failure.
 */
static struct vdfs2_task_log *get_task_log_stop(struct vdfs2_sb_info *sbi)
{
	struct vdfs2_task_log *current_task_log;
...
int vdfs2_start_transaction(struct vdfs2_sb_info *sbi)
{
	...
	if (current_task_log->r_lock_count == 0) {
		VDFS2_DEBUG_MUTEX("transaction_lock down_read");
		down_read(&sbi->snapshot_info->transaction_lock);
		VDFS2_DEBUG_MUTEX("transaction_lock down_read success");
...
int vdfs2_stop_transaction(struct vdfs2_sb_info *sbi)
{
	...
	VDFS2_BUG_ON(!current_task_log->r_lock_count);

	if (current_task_log->r_lock_count == 1) {
		VDFS2_DEBUG_MUTEX("transaction_lock up_read");
...
static __u64 validate_base_table(struct vdfs2_base_table *table)
{
	__u32 checksum, on_disk_checksum;
	char is_not_base_table;

	is_not_base_table = strncmp(table->descriptor.signature,
			VDFS2_SNAPSHOT_BASE_TABLE,
			sizeof(VDFS2_SNAPSHOT_BASE_TABLE) - 1);
	/* signature not match */
	...
	return (checksum != on_disk_checksum) ? 0 :
			VDFS2_SNAPSHOT_VERSION(table);

}

static __u64 validate_extended_table(struct vdfs2_extended_table *table)
{
	__u32 checksum, on_disk_checksum;
	char is_not_ext_table;

	is_not_ext_table = (strncmp(table->descriptor.signature,
			VDFS2_SNAPSHOT_EXTENDED_TABLE,
			sizeof(VDFS2_SNAPSHOT_EXTENDED_TABLE) - 1));
	/* signature not match */
	if (is_not_ext_table)
		return 0;
...
static __u64 parse_extended_table(struct vdfs2_sb_info *sbi,
		struct vdfs2_base_table *base_table,
		struct vdfs2_extended_table *extended_table)
{
	int count;
	__u32 rec_count = le32_to_cpu(extended_table->records_count);
	struct vdfs2_extended_record *record;
	__le64 *table;
	__u64 last_table_index;

	record = (struct vdfs2_extended_record *)((void *)extended_table +
			sizeof(struct vdfs2_extended_table));

	for (count = 0; count < rec_count; count++) {

		table = VDFS2_GET_TABLE(sbi, record->object_id);
		last_table_index = VDFS2_LAST_TABLE_INDEX(sbi,
				record->object_id);
...
static __u64 get_base_table_size(struct vdfs2_base_table *base_table)
{
	__u64 size = 0;
...
static void build_snapshot_bitmaps(struct vdfs2_sb_info *sbi)
{
	struct vdfs2_snapshot_info *snapshot = sbi->snapshot_info;
	...
	for (count = VDFS2_FSFILE; count <= VDFS2_LSFILE; count++) {
		table_offset = VDFS2_GET_TABLE_OFFSET(sbi, count);
		if (!table_offset)
			continue;

		table = VDFS2_GET_TABLE(sbi, count);
		order = is_tree(count) ? sbi->log_blocks_in_leb :
				sbi->log_blocks_in_page;
		for (page_index = 0; page_index <=
			VDFS2_LAST_TABLE_INDEX(sbi, count); page_index++) {
...
static int restore_exsb(struct vdfs2_sb_info *sbi, __u64 version)
{
	struct vdfs2_extended_super_block *exsb, *exsb_copy;
	__u64 exsb_version, exsb_copy_version;
	int ret = 0;

	exsb = VDFS2_RAW_EXSB(sbi);
	exsb_copy = VDFS2_RAW_EXSB_COPY(sbi);
	exsb_version = VDFS2_EXSB_VERSION(exsb);
	exsb_copy_version = VDFS2_EXSB_VERSION(exsb_copy);
	...
	if (version == exsb_version) {
		memcpy(exsb_copy, exsb,
			sizeof(struct vdfs2_extended_super_block));
...
static int get_table_version(struct vdfs2_sb_info *sbi,
	...
	*version = 0;
	...
	ret = vdfs2_table_IO(sbi, pages, length, REQ_OP_READ, &start);
	if (ret)
		goto exit;
...
static int load_base_table(struct vdfs2_sb_info *sbi,
		sector_t *ext_table_start, __u64 *table_version_out)
{
	struct vdfs2_extended_super_block *exsb = VDFS2_RAW_EXSB(sbi);
	struct vdfs2_extended_super_block *exsb_copy = VDFS2_RAW_EXSB_COPY(sbi);
	struct vdfs2_snapshot_info *snapshot = sbi->snapshot_info;
...
static int load_extended_table(struct vdfs2_sb_info *sbi, sector_t start)
{
	struct vdfs2_snapshot_info *snapshot = sbi->snapshot_info;
	int ret = 0;
	...
	ret = vdfs2_table_IO(sbi, snapshot->extended_table, 8, REQ_OP_READ,
			&start);
	return ret;
exit:
...
{
	struct vdfs2_extended_super_block *exsb = VDFS2_RAW_EXSB(sbi);
	struct vdfs2_snapshot_info *snapshot;
	int ret = 0;
	...
	for (count = 0; count < VDFS2_SNAPSHOT_EXT_TABLES; count++) {
		unsigned long size;
		struct vdfs2_base_table *base_table = snapshot->base_t;
		base_table_mount_count = VDFS2_MOUNT_COUNT(base_table_version);
		base_table_sync_count = VDFS2_SYNC_COUNT(base_table_version);
		...
		ext_table_version = validate_extended_table(
				...
				snapshot->extended_t);
		ext_table_mount_count = VDFS2_MOUNT_COUNT(ext_table_version);
		ext_table_sync_count = VDFS2_SYNC_COUNT(ext_table_version);
		if ((ext_table_mount_count != base_table_mount_count) ||
				(ext_table_sync_count !=
						base_table_sync_count + 1))
			break;
		size = parse_extended_table(sbi,
			(struct vdfs2_base_table *) snapshot->base_t,
			(struct vdfs2_extended_table *) snapshot->extended_t);

		base_table_version = VDFS2_SNAPSHOT_VERSION(base_table);
	...
	memset(snapshot->extended_t, 0x0, sizeof(struct vdfs2_extended_table));
	return ret;
...
static __u64 get_new_meta_iblock(struct vdfs2_sb_info *sbi, ino_t ino_n)
{
...
static void move_iblock(struct vdfs2_sb_info *sbi, ino_t ino_n,
		__u64 current_iblock, pgoff_t page_index, __u64 *new_iblock)
{
	struct vdfs2_snapshot_info *snapshot = sbi->snapshot_info;
	unsigned long *next_snapshot_bitmap = snapshot->next_snapshot_bitmap;
	unsigned long position;
	__le64 *table = VDFS2_GET_TABLE(sbi, ino_n);
	__u64 last_table_index = VDFS2_LAST_TABLE_INDEX(sbi, ino_n);
	__u64 table_index;
	int nr;
...
static void update_extended_table(struct vdfs2_sb_info *sbi, ino_t ino_n,
		sector_t page_index, __u64 new_iblock)
{
	struct vdfs2_snapshot_info *snapshot = sbi->snapshot_info;
	struct vdfs2_extended_table *ext_table = snapshot->extended_t;
	...
	record = snapshot->extended_t + sizeof(struct vdfs2_extended_table) +
			rec_count * sizeof(struct vdfs2_extended_record);
		...
		pgoff_t page_index)
{
	...
	BUG_ON((object_id < VDFS2_FSFILE) || (object_id > VDFS2_LSFILE));
	/* inconsistent lock state */
	BUG_ON(sbi->snapshot_info->flags);

	ret = vdfs2_get_meta_iblock(sbi, object_id, page_index,
			&meta_iblock);
	BUG_ON(ret);
...
static int get_bnode_size_in_pages(struct vdfs2_sb_info *sbi, ino_t i_ino)
{
	switch (i_ino) {
		...
		int page_count)
{
	...
	do {
		...
		BUG_ON(vdfs2_get_meta_iblock(sbi, i_ino, pages[count]->index,
				&meta_iblock));
		/* page must be moved */
		...
		int lock)
{
	...
	vdfs2_add_chunk_no_lock(sbi, i_ino, page->index);
	if (lock)
...
void vdfs2_add_chunk_bnode(struct vdfs2_sb_info *sbi, struct page **pages)
{
	...
	down_write(&snapshot->tables_lock);
	vdfs2_add_chunk_no_lock(sbi, i_ino, pages[0]->index);
	up_write(&snapshot->tables_lock);
}
 ...
 *			then expand file;
 * @param [in]	sbi	VDFS2 run-time superblock
 * @return		none
 */
		...
		pgoff_t page_index, char *is_new)
{
	...
	last_table_index = VDFS2_LAST_TABLE_INDEX(sbi, object_id);
	if (is_tree(object_id))
 ...
 * @return		none
 */
static void fill_descriptor_and_calculate_crc(struct vdfs2_sb_info *sbi,
		void *table, enum snapshot_table_type type)
{
	struct vdfs2_snapshot_info *snapshot = sbi->snapshot_info;
	struct vdfs2_snapshot_descriptor *descriptor = table;
	struct vdfs2_extended_super_block *exsb = VDFS2_RAW_EXSB(sbi);
	int count;
	__u32 *checksum;

	if (type == SNAPSHOT_BASE_TABLE) {
		struct vdfs2_base_table *base_table = table;
		__u64 last_table_index;

		memcpy((void *)&descriptor->signature, VDFS2_SNAPSHOT_BASE_TABLE,
				sizeof(VDFS2_SNAPSHOT_BASE_TABLE) - 1);

		base_table->descriptor.checksum_offset = 0;
		for (count = VDFS2_FSFILE; count <= VDFS2_LSFILE; count++) {
			last_table_index = VDFS2_LAST_TABLE_INDEX(sbi, count);
		...
		struct vdfs2_extended_table *ext_table = table;
		...
		ext_table->descriptor.checksum_offset = sizeof(*ext_table) +
				sizeof(struct vdfs2_extended_record) *
...
#define VDFS2_SNAPSHOT_TABLES_COUNT 8
...
int vdfs2_update_translation_tables(struct vdfs2_sb_info *sbi)
{
	...
	if (snapshot->exteneded_table_count >= VDFS2_SNAPSHOT_TABLES_COUNT ||
	...
	descriptor = (struct vdfs2_snapshot_descriptor *)mapped_table;

	table_size = le32_to_cpu(descriptor->checksum_offset) + CRC32_SIZE;
	sectors_count = (DIV_ROUND_UP(table_size, SECTOR_SIZE));

	ret = vdfs2_table_IO(sbi, table, sectors_count,
			REQ_OP_WRITE | REQ_PREFLUSH | REQ_FUA, &isector);
	snapshot->isector = isector;
	snapshot->sync_count++;
 ...
 */
static __u32 calculate_expand_step(struct vdfs2_sb_info *sbi)
{
 ...
 *			expand_block_count
 * @param [in]	sbi	VDFS2 run-time superblock
 * @param [in]	extents			VDFS2 extents massive
 ...
 * @return		Returns 0 on success, errno on failure.
 */
static int expand_area(struct vdfs2_sb_info *sbi, struct vdfs2_extent *extents,
		...
		__le32 *tbc)
{
	...
	start_offset = vdfs2_fsm_get_free_block(sbi, start_offset,
...
static unsigned int calculate_max_table_size(struct vdfs2_sb_info *sbi)
{
	struct vdfs2_extended_super_block *exsb = VDFS2_RAW_EXSB(sbi);
	__u32 meta_tbc = le32_to_cpu(exsb->meta_tbc);

	return sizeof(struct vdfs2_base_table) + sizeof(__le64) * (meta_tbc >> 1)
			+ CRC32_SIZE;
}

#if 0
static unsigned int calculate_base_table_size(struct vdfs2_sb_info *sbi)
{
	int count;
	...
	size += sizeof(struct vdfs2_base_table) + CRC32_SIZE;
		...
		sector_t new_last_table_index)
{
	...
	struct vdfs2_base_table *base_table = snapshot->base_t;
	__le64 *table = VDFS2_GET_TABLE(sbi, ino_n);
	__u64 last_table_index = VDFS2_LAST_TABLE_INDEX(sbi, ino_n), offset;
	int count;
	...
	for (count = VDFS2_FSFILE; count <= VDFS2_LSFILE; count++) {
		if (base_table->translation_table_offsets[VDFS2_SF_INDEX(ino_n)]
		>= base_table->translation_table_offsets[VDFS2_SF_INDEX(count)])
			continue;
		offset = le64_to_cpu(base_table->
			translation_table_offsets[VDFS2_SF_INDEX(count)]);
		offset += add_size;
		base_table->translation_table_offsets[VDFS2_SF_INDEX(count)] =
			cpu_to_le64(offset);
	}
...
*/
	...
	base_table->last_page_index[VDFS2_SF_INDEX(ino_n)] =
...
static int expand_translation_tables(struct vdfs2_sb_info *sbi, ino_t ino_n,
		...
		__u64 new_table_index)
{
	...
	new_blocks_count = new_table_index - VDFS2_LAST_TABLE_INDEX(sbi, ino_n);
