 ...
 * build warnings */
static int xattrtree_insert(struct vdfs2_btree *tree, u64 object_id,
		const char *name, size_t val_len, const void *value)
{
	...
	if (name_len >= VDFS2_XATTR_NAME_MAX_LEN ||
			val_len >= VDFS2_XATTR_VAL_MAX_LEN) {
		VDFS2_ERR("xattr name or val too long");
		return -EINVAL;
	}
	...
	memcpy(key->gen_key.magic, VDFS2_XATTR_REC_MAGIC,
		strlen(VDFS2_XATTR_REC_MAGIC));
	...
	ret = vdfs2_btree_insert(tree, insert_data);
	kfree(insert_data);
...
{
	struct vdfs2_xattrtree_key *key1, *key2;
	int diff;


	key1 = container_of(__key1, struct vdfs2_xattrtree_key, gen_key);
	key2 = container_of(__key2, struct vdfs2_xattrtree_key, gen_key);
...
static struct vdfs2_xattrtree_key *xattrtree_alloc_key(u64 object_id,
		const char *name)
{
	struct vdfs2_xattrtree_key *key;
	int name_len = strlen(name);
...
static struct vdfs2_xattrtree_record *vdfs2_xattrtree_find(struct vdfs2_btree
	...
	enum vdfs2_get_bnode_mode mode)
{
	...
	record = (struct vdfs2_xattrtree_record *) vdfs2_btree_find(btree,
			&key->gen_key, mode);
	if (IS_ERR(record))
		goto exit;
...
#ifndef USER_SPACE
static int xattrtree_remove_record(struct vdfs2_btree *tree, u64 object_id,
		const char *name)
{
	struct vdfs2_xattrtree_key *key;
	int ret;
	...
	ret = vdfs2_btree_remove(tree, &key->gen_key);

	return ret;
}
#endif

static int xattrtree_get_next_record(struct vdfs2_xattrtree_record *record)
{
	return vdfs2_get_next_btree_record((struct vdfs2_btree_gen_record *)
			record);
}

static struct vdfs2_xattrtree_record *xattrtree_get_first_record(
		struct vdfs2_btree *tree, u64 object_id,
		enum vdfs2_get_bnode_mode mode)
{
	struct vdfs2_xattrtree_record *record;
	int ret = 0;

	record = vdfs2_xattrtree_find(tree, object_id, "", mode);
...
{
	struct vdfs2_xattrtree_record *record = NULL;
	struct vdfs2_xattrtree_key *rm_key =
	...
	while (!ret) {
		vdfs2_start_transaction(tree->sbi);
		...
		if (IS_ERR(record)) {
			...
			vdfs2_stop_transaction(tree->sbi);
			break;
		}
		...
		ret = vdfs2_btree_remove(tree, &rm_key->gen_key);
		mutex_w_unlock(tree->rw_tree_lock);
		vdfs2_stop_transaction(tree->sbi);
	}
                ...
                const void *value, size_t size, int flags)
{
	...
	if (strlen(name) >= VDFS2_XATTR_NAME_MAX_LEN ||
			size >= VDFS2_XATTR_VAL_MAX_LEN)
		return -EINVAL;
	...
	record = vdfs2_xattrtree_find(sbi->xattr_tree, inode->i_ino, name,
			VDFS2_BNODE_MODE_RW);
...
ssize_t vdfs2_listxattr(struct dentry *dentry, char *buffer, size_t buf_size)
{
	struct inode *inode = dentry->d_inode;
	struct vdfs2_sb_info *sbi = VDFS2_SB(inode->i_sb);
	struct vdfs2_xattrtree_record *record;
...
int update_has_quota(struct vdfs2_sb_info *sbi, u64 ino, int index)
{
	...
	mutex_w_lock(sbi->xattr_tree->rw_tree_lock);
	record = vdfs2_xattrtree_find(sbi->xattr_tree, ino,
			QUOTA_HAS_XATTR, VDFS2_BNODE_MODE_RW);
	if (IS_ERR(record)) {
		mutex_w_unlock(sbi->xattr_tree->rw_tree_lock);
		return PTR_ERR(record);
	}
	...
	vdfs2_mark_bnode_dirty(VDFS2_BTREE_REC_I
...
#ifdef USER_SPACE
void dummy_xattrtree_record_init(struct vdfs2_raw_xattrtree_record *xattr_record)
{
	...
	name_len = strlen(VDFS2_XATTRTREE_ROOT_REC_NAME);
	...
	if (VDFS2_XATTR_NAME_MAX_LEN >= name_len)
		strncpy(xattr_record->key.name, VDFS2_XATTRTREE_ROOT_REC_NAME,
				name_len);
}

static void xattrtree_init_root_bnode(struct vdfs2_bnode *root_bnode)
{
	struct vdfs2_raw_xattrtree_record xattr_record;

	vdfs2_temp_stub_init_new_node_descr(root_bnode, VDFS2_NODE_LEAF);
	dummy_xattrtree_record_init(&xattr_record);
	vdfs2_temp_stub_insert_into_node(root_bnode, &xattr_record, 0);
}

int init_xattrtree(struct vdfs2_sb_info *sbi)
{
	int ret = 0;
	struct vdfs2_tools_btree_info *xattr_btree = &sbi->xattrtree;
	struct vdfs2_bnode *root_bnode = 0;

	log_activity("Create xattr tree");

	xattr_btree->tree.sub_system_id = VDFS2_XATTR_TREE_INO;
	xattr_btree->tree.subsystem_name = "XATTR TREE";
	ret = btree_init(sbi, xattr_btree, VDFS2_BTREE_XATTRS,
			sizeof(struct vdfs2_xattrtree_key) +
			sizeof(struct vdfs2_raw_xattrtree_record));

	if (ret)
		goto error_exit;
	xattr_btree->vdfs2_btree.comp_fn = vdfs2_xattrtree_cmpfn;
	sbi->xattr_tree = &xattr_btree->vdfs2_btree;
	/* Init root bnode */
	root_bnode = vdfs2_alloc_new_bnode(&xattr_btree->vdfs2_btree);
	...
	util_update_crc(xattr_btree->vdfs2_btree.head_bnode->data,
...
int get_set_xattrs(struct vdfs2_sb_info *sbi, char *path, u64 object_id,
		int has_quota, __u64 curr_size)
{
	...
	while (len > 0) {
		...
		ret = xattrtree_insert(&sbi->xattrtree.vdfs2_btree, object_id,
				name, size, val);
		if (ret) {
...
int unpack_xattr(struct vdfs2_btree *xattr_tree, char *path, u64 object_id)
{
	int ret = 0;
	char name[VDFS2_FULL_PATH_LEN];
	struct vdfs2_xattrtree_record *record = xattrtree_get_first_record(
			xattr_tree, object_id, VDFS2_BNODE_MODE_RW);
	if (IS_ERR(record))
		goto exit;
