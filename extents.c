...
{
	struct vdfs2_exttree_key *key1, *key2;

	key1 = container_of(__key1, struct vdfs2_exttree_key, gen_key);
	key2 = container_of(__key2, struct vdfs2_exttree_key, gen_key);
...
struct vdfs2_exttree_key *vdfs2_get_exttree_key(void)
{
	struct vdfs2_exttree_key *key =
			malloc(sizeof(struct vdfs2_exttree_key));
...
int vdfs2_exttree_get_next_record(struct vdfs2_exttree_record *record)
{
	...
	struct vdfs2_exttree_lrecord *raw_record =
		__vdfs2_get_next_btree_record(&bnode, &pos);
 ...
 */
static struct vdfs2_exttree_record *vdfs2_exttree_find_record_nonstrict(
		struct vdfs2_sb_info *sbi, __u64 object_id, sector_t iblock,
		enum vdfs2_get_bnode_mode mode)
{
	struct vdfs2_exttree_key *key = NULL;
	void *err_ret = NULL;
	struct vdfs2_exttree_record *record;
	struct vdfs2_btree *btree = sbi->extents_tree;

	key = vdfs2_get_exttree_key();
	if (IS_ERR(key)) {
		err_ret = key;
		return err_ret;
	}
	...
	record = (struct vdfs2_exttree_record *)
		vdfs2_btree_find(btree, &key->gen_key, mode);
	if (IS_ERR(record)) {
		err_ret = (void *) record;
		goto err_exit;
	}
...
/**
 * @brief		Behaves like vdfs2_exttree_find_record_nonstrict, but
 ...
 * @return		resulting record.
 */
static struct vdfs2_exttree_record *vdfs2_exttree_find_record_strict_obj(
		struct vdfs2_sb_info *sbi, __u64 object_id, sector_t iblock,
		enum vdfs2_get_bnode_mode mode)
{
	struct vdfs2_exttree_record *record;

	record = vdfs2_exttree_find_record_nonstrict(sbi, object_id, iblock,
			mode);
		...
		enum vdfs2_get_bnode_mode mode)
{
	int ret = 0;
	struct vdfs2_exttree_record *record =
		vdfs2_exttree_find_record_nonstrict(sbi, object_id, 0, mode);
...
{
	struct vdfs2_exttree_record *record =
		vdfs2_exttree_find_record_strict_obj(sbi, object_id,
...
int vdfs2_exttree_remove(struct vdfs2_btree *btree, __u64 object_id,
		sector_t iblock)
{
	struct vdfs2_exttree_key *key = vdfs2_get_exttree_key();
	int ret = 0;
	...
	key->object_id = cpu_to_le64(object_id);
	key->iblock = cpu_to_le64(iblock);
	ret = vdfs2_btree_remove(btree, (struct vdfs2_generic_key *)key);
...
{
	struct vdfs2_exttree_lrecord *record;
	int ret = 0;

	record = kzalloc(sizeof(struct vdfs2_exttree_lrecord), GFP_KERNEL);
	if (!record)
		return -ENOMEM;

	/* Fill the key */
	memcpy(record->key.gen_key.magic, VDFS2_EXTTREE_KEY_MAGIC,
			sizeof(VDFS2_EXTTREE_KEY_MAGIC) - 1);
	...
	ret = vdfs2_btree_insert(sbi->extents_tree, record);

	VDFS2_DEBUG_INO("exit with __FUNCTION__ %d", ret);
	kfree(record);
	return ret;
}
...
{
	struct vdfs2_fork_info *ifork = &VDFS2_I(inode)->fork;
	...
	vdfs2_lfork_to_rfork(lfork, ifork);
	return 0;
}
...
void vdfs2_form_fork(struct vdfs2_fork *lfork, struct inode *inode)
{
	...
	memset(lfork, 0, sizeof(struct vdfs2_fork));
...
{
	memset(key, 0, sizeof(struct vdfs2_exttree_key));
}

int vdfs2_exttree_cache_init(void)
{
	extents_tree_key_cachep = kmem_cache_create("vdfs2_exttree_key",
		sizeof(struct vdfs2_exttree_key), 0,
		SLAB_HWCACHE_ALIGN, exttree_key_ctor);
	if (!extents_tree_key_cachep) {
		VDFS2_ERR("failed to initialize extents tree key cache\n");
		return -ENOMEM;
	}
	return 0;
}
		...
		sector_t iblock, struct vdfs2_extent_info *result)
{
	...
	record = vdfs2_exttree_find_record_strict_obj(sbi, object_id, iblock,
			VDFS2_BNODE_MODE_RO);
	if (IS_ERR(record)) {
		...
		unsigned long object_id, struct vdfs2_extent_info *extent)
{
	...
	ret = vdfs2_exttree_add(sbi, object_id, extent);
...
{
	struct vdfs2_sb_info *sbi = VDFS2_SB(inode_info->vfs_inode.i_sb);
	__u64 object_id  = inode_info->vfs_inode.i_ino;
	struct vdfs2_extent_info extent_info;
	int ret = 0;
	sector_t res_block;

	ret = vdfs2_exttree_get_extent(sbi, object_id, iblock, &extent_info);
	if (ret)
		return 0;
...
int vdfs2_runtime_extent_add(sector_t iblock, sector_t alloc_hint,
	...
	struct vdfs2_runtime_extent_info *entry, *next_entry;
	struct vdfs2_runtime_extent_info *ext = NULL;
	list_for_each_safe(ptr, next, list) {
		...
		if ((entry->iblock + entry->block_count) == iblock) {
			...
			if (next != list) {
				next_entry = list_entry(next,
					struct vdfs2_runtime_extent_info, list);
		...
		/* insert new extent */
			...
			ext = kzalloc(sizeof(struct vdfs2_runtime_extent_info),
				GFP_KERNEL);
	...
	ext = kzalloc(sizeof(struct vdfs2_runtime_extent_info), GFP_KERNEL);
...
int vdfs2_runtime_extent_del(sector_t iblock, struct list_head *list)
{
	struct list_head *ptr, *next;
	...
	list_for_each_safe(ptr, next, list) {
		...
		/* middle of chunk */
			...
			((entry->iblock + entry->block_count) > iblock)) {
			struct vdfs2_runtime_extent_info *new = kzalloc(
				sizeof(struct vdfs2_runtime_extent_info),
				GFP_KERNEL);
			if (new == NULL)
				return -ENOMEM;
			new->iblock = iblock + 1;
		...
		struct list_head *list)
{
	...
	list_for_each_safe(ptr, next, list) {
		entry = list_entry(ptr, struct vdfs2_runtime_extent_info,
			list);
