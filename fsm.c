...
{
	struct vdfs2_fsm_info *fsm = sbi->fsm_info;
	...
	if (alignment) {
		...
		VDFS2_BUG_ON(*length_in_blocks & (blocks_per_superpage - 1));
	...
	if (block_offset) {
		...
		do_div(start_page, VDFS2_BIT_BLKSIZE(sbi->block_size,
				FSM_BMP_MAGIC_LEN));
		/* calculate end block */
		do_div(end_page, VDFS2_BIT_BLKSIZE(sbi->block_size,
				FSM_BMP_MAGIC_LEN));
		/* index of start page */
		...
		VDFS2_BUG_ON(block_offset + *length_in_blocks >
					(fsm->length_in_bytes << 3));
		VDFS2_BUG_ON(vdfs2_set_bits(fsm->data, fsm->page_count *
...
exit:
	...
	if (block_offset)
		for (index = start_page; index <= end_page; index++)
			vdfs2_add_chunk_bitmap(sbi, fsm->pages[index], lock);
 ...
 * @return	void
 */
static int fsm_free_block_chunk(struct vdfs2_fsm_info *fsm,
	__u64 block_offset, __u32 length_in_blocks, int da)
{
	...
	/* calculate start block */
	do_div(start_page, VDFS2_BIT_BLKSIZE(fsm->sbi->block_size,
			FSM_BMP_MAGIC_LEN));
	/* calculate end block */
	do_div(end_page, VDFS2_BIT_BLKSIZE(fsm->sbi->block_size,
			FSM_BMP_MAGIC_LEN));
	/* index of start page */
	start_page = (unsigned int)start_page >>\
	...
	/* clear bits */
	err = vdfs2_clear_bits(fsm->data, fsm->page_count * PAGE_SIZE,
	...
	fsm->sbi->next_free_blocks += length_in_blocks;
	for (i = 0; i < length_in_blocks; i++) {
		...
		if (!--fsm->sbi->erase_blocks_counters[erase_block_num]) {
			...
			if (rc && rc != -EOPNOTSUPP) {
				VDFS2_ERR("sb_issue_discard %d %llu %u", rc,
						erase_block_num,
	...
	for (page_index = start_page; page_index <= end_page; page_index++)
		vdfs2_add_chunk_bitmap(fsm->sbi, fsm->pages[page_index], 1);
 ...
 * @return			Returns 0 on success, -1 on failure.
 */
static int fsm_insert(struct vdfs2_fsm_info *fsm,
			__u64 block_offset, __u64 length)
{
	struct  vdfs2_fsm_btree *btree = &fsm->tree;
	struct fsm_btree_key key;
	key.block_offset = block_offset;
	key.length = length;
	VDFS2_DEBUG_FSM("INSERT off %llu\tlen %llu ",
...
int fsm_get_tree(struct vdfs2_fsm_info *fsm)
{
	...
	__u64 bits_count;
	struct vdfs2_sb_info *sbi = fsm->sbi;
	u64 total_size = (sbi->total_leb_count << sbi->log_blocks_in_leb)
			* sbi->block_size;
	int erase_block_couters_size = (total_size >> sbi->log_erase_block_size)
			+ 1;

	bits_count = sbi->total_leb_count << sbi->log_blocks_in_leb;
	memset(sbi->erase_blocks_counters, 0, sizeof(u32) *
			erase_block_couters_size);

	while (pos + 1 < bits_count) {
		switch (mode) {
		case FIND_CLEAR_BIT:
			free_start = vdfs2_find_next_zero_bit(fsm->data,
			...
			break;
		case FIND_SET_BIT:
			free_end = vdfs2_find_next_bit(fsm->data, bits_count,
 ...
 */
int vdfs2_fsm_build_management(struct super_block *sb)
{
	struct vdfs2_fsm_info *fsm;
	...
	if (sbi->log_erase_block_size == 0) {
		VDFS2_ERR("wrong erase block size");
		return -EINVAL;
	}
	...
	fsm = kzalloc(sizeof(struct vdfs2_fsm_info), GFP_KERNEL);
	if (!fsm)
		return -ENOMEM;
	...
	fsm->free_space_start = le64_to_cpu(VDFS2_RAW_EXSB(sbi)->
			volume_body.begin);
	mutex_init(&fsm->lock);
	...
	fsm->bitmap_inode = vdfs2_special_iget(sb, VDFS2_SPACE_BITMAP_INO);
	...
	err = vdfs2_read_or_create_pages(sbi, fsm->bitmap_inode, fsm->pages, 0,
			fsm->page_count, 0);
	if (err)
 ...
 * @return		void
 */
void vdfs2_fsm_destroy_management(struct super_block *sb)
{
	struct vdfs2_sb_info *sbi = sb->s_fs_info;
	struct vdfs2_fsm_info *fsm = sbi->fsm_info;
	int page_index = 0;

	VDFS2_BUG_ON(!fsm->data || !fsm->pages);
