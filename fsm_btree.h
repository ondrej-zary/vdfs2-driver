...
#define VDFS2_FSM_BTREE_ORDER	32
 ...
 */
struct vdfs2_fsm_btree {
	/* Order */
	unsigned int order;
	/* Root pointer */
	struct vdfs2_fsm_btree_node *root;
 ...
 */
__u64 fsm_btree_lookup(struct vdfs2_fsm_btree *btree, __u64 off, __u32 *len,
		int can_return_less);

/**
 * @brief	Function is used to put a node to B-tree.
 */
int fsm_btree_free(struct vdfs2_fsm_btree *btree, __u64 off, __u64 len);

/**
 * @brief	Function used to insert node into a B-tree.
 */
int fsm_btree_insert(struct  vdfs2_fsm_btree *btree, void *key_value);

/**
 * @brief	Function is used to remove a node from a  B-tree.
 */
int fsm_btree_delete(struct  vdfs2_fsm_btree *btree, void *key_value);
