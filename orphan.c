...
int vdfs2_fsm_free_runtime_fork(struct vdfs2_inode_info *inode_info,
		struct vdfs2_fork_info *fork)
{
	int error = 0;
	int i = 0;

	while (fork->total_block_count && i < VDFS2_EXTENTS_COUNT_IN_FORK) {
		__u64 block_offset = fork->extents[i].first_block;
		__u32 length = fork->extents[i].block_count;

		error = vdfs2_fsm_put_free_block(inode_info,
...
static int clear_extents(struct vdfs2_inode_info *inode_info,
		struct vdfs2_fork_info *fork)
{
	int error = 0;
	struct vdfs2_exttree_record *extent;
	...
	while (fork->total_block_count > 0) {
		...
		extent = vdfs2_exttree_find_first_record(sbi, ino,
				VDFS2_BNODE_MODE_RO);
		if (IS_ERR(extent)) {
			...
			VDFS2_ERR("Total blocks count mismatch: inode %llu "
		...
		vdfs2_release_record((struct vdfs2_btree_gen_record *) extent);
		error = vdfs2_fsm_put_free_block(inode_info,
				block_offset, length, 0);
		if (error) {
			mutex_w_unlock(sbi->extents_tree->rw_tree_lock);
		...
		error = vdfs2_exttree_remove(sbi->extents_tree, ino, iblock);
		...
		enum orphan_inode_type type, void *loc_info)
{
	...
	case ORPHAN_SMALL_FILE:
		error = vdfs2_free_cell(sbi,
			((struct vdfs2_small_file_data_info *)loc_info)->cell);
		if (error)
			goto exit;
	...
	case ORPHAN_REGULAR_FILE:
		error = vdfs2_fsm_free_runtime_fork(inode_info, loc_info);
		if (error)
			goto exit;
	...
	error = vdfs2_cattree_remove(sbi, VDFS2_ORPHAN_INODES_INO, ino_name,
			strlen(ino_name));
	mutex_w_unlock(sbi->catalog_tree->rw_tree_lock);
	if (error)
		goto exit;

	error = vdfs2_free_inode_n(sbi, ino, 1);
...
int vdfs2_process_orphan_inodes(struct vdfs2_sb_info *sbi)
{
	...
	for (;;) {
		...
		mutex_w_lock(sbi->catalog_tree->rw_tree_lock);
		record = vdfs2_cattree_get_first_child(sbi->catalog_tree,
		...
		if (record->key->parent_id !=
				cpu_to_le64(VDFS2_ORPHAN_INODES_INO)) {
		...
		ret = vdfs2_xattrtree_remove_all(sbi->xattr_tree, ino);
		if (ret) {
			VDFS2_ERR("can not clear xattrs for ino#%llu", ino);
			break;
		}
		...
		if (test_bit(SMALL_FILE, &flags)) {
			...
			vdfs2_lfork_to_rfork(&file_rec->data_fork, &fork);
...
#endif
		ret = vdfs2_kill_orphan_inode(dummy_inode_info,
				type, loc_info);
		if (ret)
			break;
